...
class LuaCacheManager:
    def __init__(self):
        ...
        self.cache_path = os.path.join(os.path.dirname(__file__), "lua_cache.json")
        self.card_data = {}
    ...
    def _update_cache(self, all_cids, progress_callback):
        ...
        if to_parse:
            ...
            for idx, (cid, path) in enumerate(to_parse):
                ...
                try:
                    ...
                    if "aux.AddContactFusionProcedure" in content or "Contact_Fusion" in content or "ContactFusion" in content:
                        consts.add("🌀 接觸融合素材")
                    if "win" in content or "Win" in content:
                        consts.add("🏆 特殊勝利條件")

                    with self._lock:
                        self.card_data[str(cid)] = list(consts)
                except Exception:
                    with self._lock:
                        self.card_data[str(cid)] = []
...
QFrame#filter_panel { background-color: #22222a; border: 1px solid #2d2d35; border-radius: 6px; }
QLineEdit#search_box { background-color: #1e1e24; }
QListWidget#card_list { background-color: #2a2a35; border: 1px solid #4a4a5a; border-radius: 6px; outline: 0; }
QListWidget#card_list::item { border: none; padding: 0px; background: transparent; }
QListWidget#card_list::item:selected { background-color: #005f87; border-radius: 4px; }
...
class Viewer(QWidget):
    ...
    def _init_ui(self):
        ...
        left_panel.setContentsMargins(0, 15, 0, 0)
        ...
        self.search.setStyleSheet("background-color: transparent; border: 1px solid #4a4a5a; border-radius: 4px; padding: 4px; color: white;")
        ...
        filter_panel_layout.setContentsMargins(0, 10, 0, 10)
        filter_panel_layout.setSpacing(0)
        ...
        self.combo_keyword_mode.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        
        kw_layout.addWidget(self.chk_multi_keyword)
        kw_layout.addWidget(lbl_mode)
        kw_layout.addWidget(self.combo_keyword_mode)
        filter_grid.addLayout(kw_layout, 0, 0)
        ...
        self.combo_search_scope.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        scope_layout.addWidget(lbl_scope)
        scope_layout.addWidget(self.combo_search_scope)
        filter_grid.addLayout(scope_layout, 0, 1)
        ...
        self.combo_sort.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        sort_layout.addWidget(lbl_sort)
        sort_layout.addWidget(self.combo_sort)
        filter_grid.addLayout(sort_layout, 1, 0)
        ...
        self.combo_env.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        env_layout.addWidget(lbl_env)
        env_layout.addWidget(self.combo_env)
        filter_grid.addLayout(env_layout, 1, 1)

        # Row 2: 禁卡表 與 狀態
        lflist_layout = QHBoxLayout()
        lflist_layout.setContentsMargins(0, 0, 0, 0)
        lflist_layout.setSpacing(6)
        lbl_lflist = QLabel("禁卡表:")
        ...
        self.combo_lflist.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        lflist_layout.addWidget(lbl_lflist)
        lflist_layout.addWidget(self.combo_lflist)
        filter_grid.addLayout(lflist_layout, 2, 0)
        ...
        self.combo_limit_filter.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        limit_layout.addWidget(lbl_limit)
        limit_layout.addWidget(self.combo_limit_filter)
        filter_grid.addLayout(limit_layout, 2, 1)

        # Row 3: 收藏夾 與 僅顯示收藏
        folder_layout = QHBoxLayout()
        folder_layout.setContentsMargins(0, 0, 0, 0)
        folder_layout.setSpacing(4)
        lbl_folder = QLabel("收藏夾:")
        ...
        filter_grid.addLayout(folder_layout, 3, 0)

        fav_only_layout = QHBoxLayout()
        fav_only_layout.setContentsMargins(0, 0, 0, 0)
        fav_only_layout.setSpacing(6)
        self.chk_fav_only = QCheckBox("僅顯示收藏")
        self.chk_fav_only.stateChanged.connect(self.apply_filter)
        fav_only_layout.addWidget(self.chk_fav_only)
        filter_grid.addLayout(fav_only_layout, 3, 1)

        # Row 4: 顯示模式 與 每行張數
        display_layout = QHBoxLayout()
        display_layout.setContentsMargins(0, 0, 0, 0)
        display_layout.setSpacing(6)
        lbl_display = QLabel("顯示:")
        ...
        self.combo_display_mode.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        display_layout.addWidget(lbl_display)
        display_layout.addWidget(self.combo_display_mode)
        filter_grid.addLayout(display_layout, 4, 0)

        columns_layout = QHBoxLayout()
        columns_layout.setContentsMargins(0, 0, 0, 0)
        columns_layout.setSpacing(6)
        lbl_columns = QLabel("每行:")
        ...
        columns_layout.addWidget(self.combo_columns)
        filter_grid.addLayout(columns_layout, 4, 1)

        filter_panel_layout.addLayout(filter_grid)

        left_panel.addLayout(search_layout)
        left_panel.addWidget(filter_panel)

        self.lbl_count = QLabel("篩選出 : 0 / 全庫 : 0 筆")

        self.stack = QStackedWidget()
        self.list = QListWidget()
        self.list.setObjectName("card_list")
    ...
    def update_list_grid(self):
        ...
        # Subtract 12px for scrollbar (10px) and borders (2px) to get exact viewport width
        avail_width = width - 12 - margin * 2
        if cols > 1:
        ...
        # Standard card ratio is 1 : 1.455. Set card width slightly smaller than cell width
        icon_width = item_width - 8
        icon_height = int(icon_width * 1.45)
        item_height = icon_height + 22
